package com.logistics.service;

import com.logistics.entity.BOrderform;
import java.util.List;

/**
 * (BOrderform)表服务接口
 *
 * @author makejava
 * @since 2019-10-23 14:32:59
 */
public interface BOrderformService {

    /**
     * 通过ID查询单条数据
     *
     * @param orderformid 主键
     * @return 实例对象
     */
    BOrderform queryById(Integer orderformid);

    /**
     * 查询多条数据
     *
     * @param offset 查询起始位置
     * @param limit 查询条数
     * @return 对象列表
     */
    List<BOrderform> queryAllByLimit(int offset, int limit);

    /**
     * 新增数据
     *
     * @param bOrderform 实例对象
     * @return 实例对象
     */
    BOrderform insert(BOrderform bOrderform);

    /**
     * 修改数据
     *
     * @param bOrderform 实例对象
     * @return 实例对象
     */
    BOrderform update(BOrderform bOrderform);

    /**
     * 通过主键删除数据
     *
     * @param orderformid 主键
     * @return 是否成功
     */
    boolean deleteById(Integer orderformid);

}