...
(function ($) {
  ...
  function selectToDiv(str) {
    ...
    result = result.replace(/<option(.*?)<\/option>/gi, function (matcher) {
      ...
      return '<li  id="' + value[1] + '" ' + (isDisabled ? ' disabled' : ' tabindex="0"') + ' data-value="' + (value[1] || '') + '" class="dropdown-option ' + (isSelected ? 'dropdown-chose' : '') + '">' + (name[1] || '') + ' </li>';
    });
    ...
    multiChoose: function multiChoose(event) {
      ...
      $.each(_config.data, function (key, item) {
        ...
        if (item.selected) {
          ...
          _dropdown.name.push('<span class="dropdown-selected">' + item.name + '<i class="del" id="' + item.id + '  " ></i></span>');
        }
      });
    ...
    singleChoose: function singleChoose(event) {
      ...
      $.each(_config.data, function (key, item) {
        ...
        if ('' + item.id === '' + value) {
          ...
          if (item.selected) {
              _dropdown.name.push('<span class="dropdown-selected" class="Name">' + item.name + '<i class="del" id="' + item.id + '"></i></span> <input class="Custom hidden"  id="' + item.id + '"value="' + item.id + '" />');
          }
        }
      });
      ...
      _dropdown.name.push('<span class="placeholder"id="placeholder">' + _dropdown.placeholder + '</span>');
  ...
  Dropdown.prototype = {
    ...
    renderSelect: function renderSelect() {
      ...
      if (!_this.isLabelMode) {
          _this.$choseList.html($('<span class="placeholder"id="placeholder"></span>').text(_this.placeholder));
      }
