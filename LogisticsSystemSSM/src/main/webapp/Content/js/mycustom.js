 ...
 * 
 * @param {}
 *            select2 select2下拉框选择器 .class #id string
 * @param {}
 *            jsonUrl json地址 string
 * @returns {}
 */
function select2LoadData(select2, jsonUrl) {
    ...
    // 请求数据，本地数据方式绑定
    /*
	 * $.post(jsonUrl, function (data) { $(select2).select2({ data: data }); });
	 */
    $.ajax({
        type: "POST",
        url: jsonUrl,
 ...
 *            弹窗内容
 * @param showType
 *            弹窗类型(如果未填默认1; 1:info,2:success,3:warning,4:error)
 * @param showTime
 *            弹窗显示时间(单位ms; 填默认5000)
 * @param closeButton
 *            是否显示关闭按钮(显示：true; 默认true)
 * @param progressBar
 *            是否显示进度条(显示：true; 默认false)
 * @param position
 *            弹窗位置(默认1; 1：右上,2:右下,3:左下,4:左上,5:顶部全宽,6:底部全宽,7:顶部居中,8:底部居中)
 */
...
function selectLoadData(selectId, jsonData) {
    ...
    for (var k in jsonData) {
        $("#" + selectId).append('<option value="' + jsonData[k].userid + '">' + jsonData[k].username + '</option>');
    }
}
...
function BangDingXiaLaKuang(ID,url)
{
	$.post(url,
			function(data)
			{
